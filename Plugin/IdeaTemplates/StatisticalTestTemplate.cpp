...
void StatisticalTestTemplate::load(const QJsonObject &obj)
{
    ...
    if(!v.isUndefined())
    {
        m_alphaColor = QColor(v.toString());
        emit alphaColorChanged(m_alphaColor);
    }
...
QColor StatisticalTestTemplate::alphaColor() const
{
    return m_alphaColor;
}
...
void StatisticalTestTemplate::setAlphaColor(QColor alphaColor)
{
    if (m_alphaColor == alphaColor)
        return;
...
void StatisticalTestTemplate::cycleAlphaColor()
{
    QColor newColor = Magic<LabelStyle>::cast().getColor();
    setAlphaColor(newColor);
}
