...
NumbersToNumberTemplate::NumbersToNumberTemplate() :
    ...
    m_xAxisLabel("PlaceHodler (Units)")
{
...
void NumbersToNumberTemplate::load(const QJsonObject &obj)
{
    ...
    if(!v.isUndefined())
    {
        m_xAxisLabel = v.toString();
        emit xAxisLabelChanged(m_xAxisLabel);
    }
...
{
    m_numbersInput = std::dynamic_pointer_cast<NumbersDataType>(dataType);
...
QString NumbersToNumberTemplate::xAxisLabel() const
{
    return m_xAxisLabel;
}

void NumbersToNumberTemplate::setXAxisLabel(const QString &newXAxisLabel)
{
