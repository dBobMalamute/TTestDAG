...
{
    auto v_nums = m_numbersInput.lock();
    if(v_nums)
    {
        if(v_nums->numbers().size() == 0)
        {
            setStatus(IdeaStatus::StandBy, "No Input", "The input contains no numbers. It isn't possible to calculate the "
                                                     "mean of nothing");
        }
